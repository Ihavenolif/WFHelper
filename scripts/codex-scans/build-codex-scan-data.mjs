...
function parseEntries(lua, faction) {
  ...
  for (const block of blocks) {
    ...
    entries.push({ internal, name, scans: Number(scans), faction });
  }
  return entries;
}
...
const lines = sorted.map(
  (e) => `  ${JSON.stringify(e.internal)}: { name: ${JSON.stringify(e.name)}, scans: ${e.scans} },`,
);
const banner =
...
console.log(`wrote ${OUT_FILE} with ${sorted.length} enemies`);
